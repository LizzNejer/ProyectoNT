<html>
    ...
    <body>
<img src="/imagenes/consultarcli.png" width="110" height="100">	
    <center>
		...
		<form class="margen" method="DELETE">
			{{csrf_field()}}	
				...
				<table border="1" >
					...
					@foreach($resultado_clientes as $l)
						<tr>
							<td>{{	$l->nombre	}}</td>
							<td>{{	$l->apellido	}}</td>
							<td>{{	$l->cedula	}}</td>
							<td>{{	$l->correo	}}</td>	
							<td>{{	$l->telefono	}}</td>	
							<td><a href="http://localhost:8000/api/eliminar/{{	$l->id	}}" class="btn btn-warning">Eliminar<span class="glyphicon glyphicon-edit"></span></a></td>
						</tr>
					  @endforeach
				</table>
		</form>
